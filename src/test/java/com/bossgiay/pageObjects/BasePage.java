...
*/
    ...
    public static void selectDynamicByText(By parentLocator, By childLocator, String selectedTxt, WebDriver dr) {
        int childSize= dr.findElements(childLocator).size();
//        for loop to select
        for (int ind= 0; ind< childSize; ind+=1) {
            WebElement parentWE= dr.findElement(parentLocator);
            List<WebElement> childrenWE= dr.findElements(childLocator);
            if (childrenWE.get(ind).getText().equalsIgnoreCase(selectedTxt) ||
                    childrenWE.get(ind).getText().contains(selectedTxt)) {
                WebDriverWait wait= new WebDriverWait(dr, 30);
                wait.until(ExpectedConditions.visibilityOf(childrenWE.get(ind))).click();
                break;
            }
        }
    }

    public static void selectDynamicByValue(By parentLocator, By childLocator,
                                            String selectedAttribute, String selectedText, WebDriver dr) {
        int childSize= dr.findElements(childLocator).size();
//        for loop to select
        for (int ind= 0; ind< childSize; ind+=1) {
            WebElement parentWE= dr.findElement(parentLocator);
            List<WebElement> childrenWE= dr.findElements(childLocator);
            if (childrenWE.get(ind).getAttribute(selectedAttribute).equalsIgnoreCase(selectedText) ||
                    childrenWE.get(ind).getAttribute(selectedAttribute).contains(selectedText)) {
                WebDriverWait wait= new WebDriverWait(dr, 30);
                wait.until(ExpectedConditions.visibilityOf(childrenWE.get(ind))).click();
                break;
            }
